...
nodes. The total cost of the computing hardware for this cluster is calculated
in \textbf{Table \ref{xu-cost-table}}.
...
purchased new for this cluster, the hardware costs of these PCs are estimated in
\textbf{Table \ref{x86-cost-table}}.
...
at the default allocation per process. \textbf{Table \ref{lp}} lists the total
number of LPs across all processes as well as the termination time set for each
model.
...
this section. The error bars in \textbf{Figures \ref{airport}, \ref{disksim},
  \ref{raid},} and \textbf{\ref{wifi}}. show a 99\% confidence interval for the
mean simulation time.

\begin{figure}
\centering
%\includegraphics[width=0.75\textwidth]{airport}
\caption{Mean Simulation Runtimes for ROSS \textsc{airport} Model}
\label{airport}
\end{figure}

\begin{figure}
\centering
%\includegraphics[width=0.75\textwidth]{airport_median}
\caption{Median Simulation Runtimes for ROSS \textsc{airport} Model}
\label{airport-median}
\end{figure}
