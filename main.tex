...
disparity).  The much lower per node costs (on the order of \$200/node) can enable the
...
However, comparing the ODROID-based cluster performance for cost, both in terms of
...
%% ERIC: can you please insert a citation for the RapidIO solution??

%% ERIC: returning after a couple of more paragraphs.  i'm also going to drop in empty
%% cite latex commands where i think you should add citations.
...
%% ERIC: you need a citation for active messages (UUNET, GAMMA, Illinois Fast Messages,
%% etc) in the first sentence and a citation for RDMA in the last.
...
%% ERIC: more citations....i assume that liu-13 is a general article; it probably contains
%% the citations you need for each individual policy described below.
...
%% ERIC, verify the last 2 sentences that i added....
...
Figure \ref{rxe-linux} illustrates how the \verb;rxe; driver functions
...
Figure \ref{rxe-overview} shows the dataflow among the major partitions
...
Figure \ref{rxe-recv}. This function receives a socket buffer (SKB)
...
The QP Responder's architecture is illustrated in Figure
...
operations for RDMA semantics. In Figure \ref{rxe-resp}, this is
...
The QP Requester's architecture is illustrated in Figure
...
The Arbiter task is illustrated in Figure \ref{rxe-arbiter}. It
...
\verb;rxe; transport module described in Section \ref{rxe}. It
...
driver in Figure \ref{rxe-linux}. In addition to facilitating this
...
\cite{InfiniBandTARoCE-10}. Figure \ref{roce-packet} compares normal
...
the addition of the line in Figure \ref{pci_code} near the beginning of
...
to I/O devices. The final address space mapping is shown in Figure
  \ref{addrmap}.
...
estimated in Table \ref{x86-cost-table}.
...
integers. Figure \ref{sysbench-all} organizes the results of these
...
Figure \ref{sysbench-cost} compares the performance per dollar of the
...
K6/233. Figure \ref{nbench-all} shows the performance comparison for
...
Figure \ref{nbench-performance} sets a new baseline for performance:
our Cortex-A15 machines. The performance results with optimizations are used to
set the performance of the ARM processor at one in each category. Figure
...
The MPI bandwidth results in Figures \ref{npmpi-lbw} and
\ref{npmpi-hbw} mirror the latency results. Interestingly, neither TCP
...
congestion increases. Figure \ref{multipingpong-u2} supports this
...
Figures \ref{xu-lat-small} and \ref{xu-lat-large} show the
