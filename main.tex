...
termiantion condition \cite{jefferson-85}.
...
performance platforms. Proprietary interconnect solutions such as Myrinet and
...
The Transmission Control Protocol/Internet Protocol describes a packet-switched
network designed to communicate over a wide area with reliable, ordered
transmission of these packets, each of which contains one or more messages.
...
the receiving node's memory. TCP, on the other hand, would probably accomplish
...
GAMMA on the same Gigabit Ethernet hardware \cite{schneidenbach-03}. GAMMA Message throughput was
also consistently higher than TCP/IP for all message sizes. Ciaccio also found
that GAMMA message latency 
...
controller is even slower than we feared, resulting in end-to-end TCP small
message latency over 500 microseconds.
